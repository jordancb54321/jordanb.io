...
function getBookMove(history) {
  var choices = openingBook[(history || []).join(" ")];
...
function getBestMove(depth, history) {
  var bookMove = getBookMove(history);

  if (bookMove) {
    nodesSearched = 0;
    return { move: bookMove, book: true };
  }
  ...
  return { move: bestMove, book: false };
}
...
self.onmessage = function (event) {
  ...
  try {
    ...
    var result = getBestMove(Number(data.depth) || 2, data.history || []);
    ...
    self.postMessage({
      requestId: data.requestId,
      error: error.message || "Worker search failed"
    });
  }
};
